...
inputfilename   = '../data/initial_positions_nordic4.txt'
...
do i = 1, size(orders)
    ...
    ! Run simulations
    call run_all_fixed(X0, t0, tmax, timesteps, f, 'nordic_' // trim(suffix(order)))
    call run_all_variable(X0, t0, tmax, tolerances, f, 'nordic_' // trim(suffix(order)), h0)
    call run_all_special(X0, t0, tmax, tc, tolerances, f, 'nordic_' // trim(suffix(order)), h0)
