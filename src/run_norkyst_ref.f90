...
!            rk4, 'reference_norkyst_rk4_' // trim(suffix(order)) // '.hdf5')
!    ! Run simulations with dp87
!    call experiment_variable(X0, t0, tmax, tolerances_ref, f, &
!            dp87, 'reference_norkyst_dp87_' // trim(suffix(order)) // '.hdf5', h0input = 1.0_WP)
    ! Run simulations with dp87 special
    call experiment_special(X0, t0, tmax, tc, tolerances_ref, f, &
            dp87, 'reference_norkyst_dp87_special_' // trim(suffix(order)) // '.hdf5', h0input = 1.0_WP)
