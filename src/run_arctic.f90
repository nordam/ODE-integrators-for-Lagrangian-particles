...
do i = 1, size(orders)
    ...
    ! Run simulations
    call run_all_fixed(X0, t0, tmax, timesteps, f, 'arctic_' // trim(suffix(order)))
    call run_all_variable(X0, t0, tmax, tolerances, f, 'arctic_' // trim(suffix(order)), h0)
    call run_all_special(X0, t0, tmax, tc, tolerances, f, 'arctic_' // trim(suffix(order)), h0)
